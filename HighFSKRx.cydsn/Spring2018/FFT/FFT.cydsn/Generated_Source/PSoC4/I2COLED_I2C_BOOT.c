...
*
* \globalvars
*  I2COLED_applyBuffer - the flag to release the buffer with a response
...
*
* \globalvars
*  I2COLED_slRdBufSize - used to store slave read buffer size.
*  I2COLED_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  I2COLED_slWrBufIndex - used to store current index within slave
*  write buffer.
*  I2COLED_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
            ...
            if (0u != (I2COLED_I2C_SSTAT_WR_CMPLT & I2COLED_slStatus))
            {
                ...
                *count = (uint16) I2COLED_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) I2COLED_slWriteBuf,
                                              I2COLED_BYTES_TO_COPY(*count, size));
...
*
* \globalvars
*  I2COLED_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
...
static void I2COLED_I2CResposeInsert(void)
{
    if (I2COLED_CHECK_INTR_SLAVE_MASKED(I2COLED_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        ...
        else
        {
            ...
            {
                I2COLED_slRdBufIndex = I2COLED_slRdBufSize;
            }
        }
    }
}
