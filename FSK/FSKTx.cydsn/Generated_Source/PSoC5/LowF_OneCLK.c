...
/*******************************************************************************
* Function Name: LowF_OneCLK_Start
********************************************************************************
*
* Summary:
...
void LowF_OneCLK_Start(void) 
{
    ...
    LowF_OneCLK_CLKEN |= LowF_OneCLK_CLKEN_MASK;
	LowF_OneCLK_CLKSTBY |= LowF_OneCLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LowF_OneCLK_Stop
********************************************************************************
*
* Summary:
...
void LowF_OneCLK_Stop(void) 
{
    ...
    LowF_OneCLK_CLKEN &= (uint8)(~LowF_OneCLK_CLKEN_MASK);
	LowF_OneCLK_CLKSTBY &= (uint8)(~LowF_OneCLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LowF_OneCLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LowF_OneCLK_CLKEN & LowF_OneCLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LowF_OneCLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LowF_OneCLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LowF_OneCLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LowF_OneCLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LowF_OneCLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LowF_OneCLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LowF_OneCLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LowF_OneCLK_GetSourceRegister();
    uint16 oldDivider = LowF_OneCLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LowF_OneCLK_CLKEN & LowF_OneCLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LowF_OneCLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: LowF_OneCLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LowF_OneCLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: LowF_OneCLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LowF_OneCLK_MOD_SRC |= modeBitMask & (uint8)LowF_OneCLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: LowF_OneCLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LowF_OneCLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LowF_OneCLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: LowF_OneCLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LowF_OneCLK_MOD_SRC & (uint8)(LowF_OneCLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: LowF_OneCLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LowF_OneCLK_GetDividerRegister();
    uint8 oldSrc = LowF_OneCLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LowF_OneCLK_MOD_SRC |= CYCLK_SSS;
        LowF_OneCLK_MOD_SRC =
            (LowF_OneCLK_MOD_SRC & (uint8)(~LowF_OneCLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LowF_OneCLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LowF_OneCLK_MOD_SRC & LowF_OneCLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LowF_OneCLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LowF_OneCLK_PHASE = clkPhase & LowF_OneCLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LowF_OneCLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LowF_OneCLK_PHASE & LowF_OneCLK_PHASE_MASK;
}
