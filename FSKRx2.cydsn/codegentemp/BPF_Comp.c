...
uint8 BPF_Comp_initVar = 0u;

/* Internal functions definitoin */
static void BPF_Comp_trimAdjust(uint8 nibble) ;

/* static BPF_Comp_backupStruct  BPF_Comp_backup; */
#if (CY_PSOC5A)
    static BPF_Comp_LOWPOWER_BACKUP_STRUCT  BPF_Comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 BPF_Comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: BPF_Comp_Init
********************************************************************************
*
* Summary:
...
void BPF_Comp_Init(void) 
{
    ...
    BPF_Comp_SetSpeed(BPF_Comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( BPF_Comp_DEFAULT_HYSTERESIS == 0u )
        BPF_Comp_CR |= BPF_Comp_HYST_OFF;
    #else
        BPF_Comp_CR &= (uint8)(~BPF_Comp_HYST_OFF);
    #endif /* BPF_Comp_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( BPF_Comp_DEFAULT_PWRDWN_OVRD == 0u )
            BPF_Comp_CR &= (uint8)(~BPF_Comp_PWRDWN_OVRD);
        #else 
            BPF_Comp_CR |= BPF_Comp_PWRDWN_OVRD;
        #endif /* BPF_Comp_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    BPF_Comp_CR |= BPF_Comp_MX_AO;

    /* Set default sync */
    BPF_Comp_CLK &= (uint8)(~BPF_Comp_SYNCCLK_MASK);
    #if ( BPF_Comp_DEFAULT_BYPASS_SYNC == 0u )
        BPF_Comp_CLK |= BPF_Comp_SYNC_CLK_EN;
    #else
        BPF_Comp_CLK |= BPF_Comp_BYPASS_SYNC;
    #endif /* BPF_Comp_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: BPF_Comp_Enable
********************************************************************************
*
* Summary:
...
{
    BPF_Comp_PWRMGR |= BPF_Comp_ACT_PWR_EN;
    BPF_Comp_STBY_PWRMGR |= BPF_Comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(BPF_Comp_restoreReg == 1u)
        {
            BPF_Comp_CR = BPF_Comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            BPF_Comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  BPF_Comp_initVar: Is modified when this function is called for the 
...
void BPF_Comp_Stop(void) 
{
    ...
    BPF_Comp_PWRMGR &= (uint8)(~BPF_Comp_ACT_PWR_EN);
    BPF_Comp_STBY_PWRMGR &= (uint8)(~BPF_Comp_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        BPF_Comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        BPF_Comp_lowPowerBackup.compCRReg = BPF_Comp_CR;
        BPF_Comp_CR = BPF_Comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: BPF_Comp_SetSpeed
********************************************************************************
*
* Summary:
...
void BPF_Comp_SetSpeed(uint8 speed) 
{
    ...
    BPF_Comp_CR = (BPF_Comp_CR & (uint8)(~BPF_Comp_PWR_MODE_MASK)) |
                           (speed & BPF_Comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == BPF_Comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            BPF_Comp_TR = BPF_Comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            BPF_Comp_TR0 = BPF_Comp_HS_TRIM_TR0;
            BPF_Comp_TR1 = BPF_Comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            BPF_Comp_TR = BPF_Comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            BPF_Comp_TR0 = BPF_Comp_LS_TRIM_TR0;
            BPF_Comp_TR1 = BPF_Comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: BPF_Comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( BPF_Comp_WRK & BPF_Comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: BPF_Comp_trimAdjust
********************************************************************************
*
* Summary:
...
static void BPF_Comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = BPF_Comp_WRK & BPF_Comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                BPF_Comp_TR |= BPF_Comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                BPF_Comp_TR0 |= BPF_Comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                BPF_Comp_TR |= BPF_Comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                BPF_Comp_TR1 |= BPF_Comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((BPF_Comp_WRK & BPF_Comp_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: BPF_Comp_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 BPF_Comp_ZeroCal(void) 
{
    ...
    BPF_Comp_SW0 = 0x00u;
    BPF_Comp_SW2 = 0x00u;
    BPF_Comp_SW3 = tmpSW3 & (uint8)(~BPF_Comp_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = BPF_Comp_CR;
    BPF_Comp_CR |= (BPF_Comp_CAL_ON | BPF_Comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        BPF_Comp_TR = BPF_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        BPF_Comp_TR0 = BPF_Comp_DEFAULT_CMP_TRIM;
        BPF_Comp_TR1 = BPF_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (BPF_Comp_CR & BPF_Comp_PWR_MODE_MASK) == BPF_Comp_PWR_MODE_FAST)
    {
        BPF_Comp_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        BPF_Comp_trimAdjust(1u);
		BPF_Comp_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) BPF_Comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)BPF_Comp_TR1 << 8u) | (BPF_Comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: BPF_Comp_LoadTrim
********************************************************************************
*
* Summary:
...
void BPF_Comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: BPF_Comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: BPF_Comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
