...
void DelayClk_Start(void) 
{
    ...
    DelayClk_CLKEN |= DelayClk_CLKEN_MASK;
	DelayClk_CLKSTBY |= DelayClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: DelayClk_Stop
********************************************************************************
*
* Summary:
...
void DelayClk_Stop(void) 
{
    ...
    DelayClk_CLKEN &= (uint8)(~DelayClk_CLKEN_MASK);
	DelayClk_CLKSTBY &= (uint8)(~DelayClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DelayClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((DelayClk_CLKEN & DelayClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = DelayClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DelayClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DelayClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DelayClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DelayClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DelayClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DelayClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = DelayClk_GetSourceRegister();
    uint16 oldDivider = DelayClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DelayClk_CLKEN & DelayClk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DelayClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: DelayClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(DelayClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: DelayClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    DelayClk_MOD_SRC |= modeBitMask & (uint8)DelayClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: DelayClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    DelayClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DelayClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: DelayClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return DelayClk_MOD_SRC & (uint8)(DelayClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: DelayClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = DelayClk_GetDividerRegister();
    uint8 oldSrc = DelayClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DelayClk_MOD_SRC |= CYCLK_SSS;
        DelayClk_MOD_SRC =
            (DelayClk_MOD_SRC & (uint8)(~DelayClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DelayClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return DelayClk_MOD_SRC & DelayClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DelayClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    DelayClk_PHASE = clkPhase & DelayClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DelayClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return DelayClk_PHASE & DelayClk_PHASE_MASK;
}
