...
/*******************************************************************************
* Function Name: HighF_OneCLK_Start
********************************************************************************
*
* Summary:
...
void HighF_OneCLK_Start(void) 
{
    ...
    HighF_OneCLK_CLKEN |= HighF_OneCLK_CLKEN_MASK;
	HighF_OneCLK_CLKSTBY |= HighF_OneCLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HighF_OneCLK_Stop
********************************************************************************
*
* Summary:
...
void HighF_OneCLK_Stop(void) 
{
    ...
    HighF_OneCLK_CLKEN &= (uint8)(~HighF_OneCLK_CLKEN_MASK);
	HighF_OneCLK_CLKSTBY &= (uint8)(~HighF_OneCLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HighF_OneCLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HighF_OneCLK_CLKEN & HighF_OneCLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HighF_OneCLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HighF_OneCLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HighF_OneCLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HighF_OneCLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HighF_OneCLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HighF_OneCLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HighF_OneCLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HighF_OneCLK_GetSourceRegister();
    uint16 oldDivider = HighF_OneCLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HighF_OneCLK_CLKEN & HighF_OneCLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HighF_OneCLK_DIV_PTR, clkDivider);
                HighF_OneCLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HighF_OneCLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HighF_OneCLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HighF_OneCLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HighF_OneCLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HighF_OneCLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HighF_OneCLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_OneCLK_MOD_SRC |= modeBitMask & (uint8)HighF_OneCLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HighF_OneCLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_OneCLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HighF_OneCLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HighF_OneCLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_OneCLK_MOD_SRC & (uint8)(HighF_OneCLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HighF_OneCLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HighF_OneCLK_GetDividerRegister();
    uint8 oldSrc = HighF_OneCLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HighF_OneCLK_MOD_SRC |= CYCLK_SSS;
        HighF_OneCLK_MOD_SRC =
            (HighF_OneCLK_MOD_SRC & (uint8)(~HighF_OneCLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HighF_OneCLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_OneCLK_MOD_SRC & HighF_OneCLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HighF_OneCLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HighF_OneCLK_PHASE = clkPhase & HighF_OneCLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HighF_OneCLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HighF_OneCLK_PHASE & HighF_OneCLK_PHASE_MASK;
}
