 ...
 * 
 * Edits:
 *  - (JCS) Added code to Display() to display Vref used on final comparator.
 *      comparator value is changed externally from pot. Value displayed in mV.
 * =============================================================================
*/
...
char ReferenceString[ARRAY_SIZE];
...
void Display()
{
    ...
    // Postfix will display good or bad below data on screen
        ...
        //Clears current comparator reference voltage
        LCD_Char_Position(1, 5);
        char8 displayG[] = "    ";
        LCD_Char_PrintString(displayG);
        
        //Calculates and displays updated comparator reference voltage 
        uint16_t Final_Comp_Ref = ADC_GetResult32() * 19.6;
        LCD_Char_Position(1, 5);
        sprintf(ReferenceString, "%u", Final_Comp_Ref);
        LCD_Char_PrintString(ReferenceString);
    }
}//end Display()
