...
/*******************************************************************************
* Function Name: Clock_Audible_Start
********************************************************************************
*
* Summary:
...
void Clock_Audible_Start(void) 
{
    ...
    Clock_Audible_CLKEN |= Clock_Audible_CLKEN_MASK;
	Clock_Audible_CLKSTBY |= Clock_Audible_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Audible_Stop
********************************************************************************
*
* Summary:
...
void Clock_Audible_Stop(void) 
{
    ...
    Clock_Audible_CLKEN &= (uint8)(~Clock_Audible_CLKEN_MASK);
	Clock_Audible_CLKSTBY &= (uint8)(~Clock_Audible_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Audible_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Audible_CLKEN & Clock_Audible_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Audible_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Audible_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Audible__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Audible_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Audible_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Audible_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Audible_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Audible_GetSourceRegister();
    uint16 oldDivider = Clock_Audible_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Audible_CLKEN & Clock_Audible_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Audible_DIV_PTR, clkDivider);
                Clock_Audible_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Audible_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Audible_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Audible_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Audible_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Audible_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Audible_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Audible_MOD_SRC |= modeBitMask & (uint8)Clock_Audible_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Audible_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Audible_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Audible_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Audible_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Audible_MOD_SRC & (uint8)(Clock_Audible_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Audible_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Audible_GetDividerRegister();
    uint8 oldSrc = Clock_Audible_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Audible_MOD_SRC |= CYCLK_SSS;
        Clock_Audible_MOD_SRC =
            (Clock_Audible_MOD_SRC & (uint8)(~Clock_Audible_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Audible_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Audible_MOD_SRC & Clock_Audible_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Audible_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Audible_PHASE = clkPhase & Clock_Audible_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Audible_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Audible_PHASE & Clock_Audible_PHASE_MASK;
}
