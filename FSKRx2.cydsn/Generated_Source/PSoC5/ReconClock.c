...
/*******************************************************************************
* Function Name: ReconClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    ReconClock_CLKEN |= ReconClock_CLKEN_MASK;
	ReconClock_CLKSTBY |= ReconClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ReconClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ReconClock_CLKEN &= (uint8)(~ReconClock_CLKEN_MASK);
	ReconClock_CLKSTBY &= (uint8)(~ReconClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ReconClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_StopBlock(void) 
{
    if ((ReconClock_CLKEN & ReconClock_CLKEN_MASK) != 0u)
    {
...
#if defined(ReconClock__CFG3)
        CLK_DIST_AMASK = ReconClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ReconClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ReconClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ReconClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ReconClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ReconClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ReconClock_CLKSTBY &= (uint8)(~ReconClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ReconClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ReconClock_GetSourceRegister();
    uint16 oldDivider = ReconClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ReconClock_CLKEN & ReconClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ReconClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ReconClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ReconClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(ReconClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ReconClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_SetModeRegister(uint8 modeBitMask) 
{
    ReconClock_MOD_SRC |= modeBitMask & (uint8)ReconClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ReconClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_ClearModeRegister(uint8 modeBitMask) 
{
    ReconClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ReconClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ReconClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ReconClock_GetModeRegister(void) 
{
    return ReconClock_MOD_SRC & (uint8)(ReconClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ReconClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ReconClock_GetDividerRegister();
    uint8 oldSrc = ReconClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ReconClock_MOD_SRC |= CYCLK_SSS;
        ReconClock_MOD_SRC =
            (ReconClock_MOD_SRC & (uint8)(~ReconClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ReconClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ReconClock_GetSourceRegister(void) 
{
    return ReconClock_MOD_SRC & ReconClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ReconClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ReconClock_SetPhaseRegister(uint8 clkPhase) 
{
    ReconClock_PHASE = clkPhase & ReconClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ReconClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ReconClock_GetPhaseRegister(void) 
{
    return ReconClock_PHASE & ReconClock_PHASE_MASK;
}
