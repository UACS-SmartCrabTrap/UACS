...
uint8 Out_Comp_initVar = 0u;

/* Internal functions definitoin */
static void Out_Comp_trimAdjust(uint8 nibble) ;

/* static Out_Comp_backupStruct  Out_Comp_backup; */
#if (CY_PSOC5A)
    static Out_Comp_LOWPOWER_BACKUP_STRUCT  Out_Comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Out_Comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Out_Comp_Init
********************************************************************************
*
* Summary:
...
void Out_Comp_Init(void) 
{
    ...
    Out_Comp_SetSpeed(Out_Comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Out_Comp_DEFAULT_HYSTERESIS == 0u )
        Out_Comp_CR |= Out_Comp_HYST_OFF;
    #else
        Out_Comp_CR &= (uint8)(~Out_Comp_HYST_OFF);
    #endif /* Out_Comp_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Out_Comp_DEFAULT_PWRDWN_OVRD == 0u )
            Out_Comp_CR &= (uint8)(~Out_Comp_PWRDWN_OVRD);
        #else 
            Out_Comp_CR |= Out_Comp_PWRDWN_OVRD;
        #endif /* Out_Comp_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Out_Comp_CR |= Out_Comp_MX_AO;

    /* Set default sync */
    Out_Comp_CLK &= (uint8)(~Out_Comp_SYNCCLK_MASK);
    #if ( Out_Comp_DEFAULT_BYPASS_SYNC == 0u )
        Out_Comp_CLK |= Out_Comp_SYNC_CLK_EN;
    #else
        Out_Comp_CLK |= Out_Comp_BYPASS_SYNC;
    #endif /* Out_Comp_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Out_Comp_Enable
********************************************************************************
*
* Summary:
...
{
    Out_Comp_PWRMGR |= Out_Comp_ACT_PWR_EN;
    Out_Comp_STBY_PWRMGR |= Out_Comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Out_Comp_restoreReg == 1u)
        {
            Out_Comp_CR = Out_Comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Out_Comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  Out_Comp_initVar: Is modified when this function is called for the 
...
void Out_Comp_Stop(void) 
{
    ...
    Out_Comp_PWRMGR &= (uint8)(~Out_Comp_ACT_PWR_EN);
    Out_Comp_STBY_PWRMGR &= (uint8)(~Out_Comp_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Out_Comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        Out_Comp_lowPowerBackup.compCRReg = Out_Comp_CR;
        Out_Comp_CR = Out_Comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Out_Comp_SetSpeed
********************************************************************************
*
* Summary:
...
void Out_Comp_SetSpeed(uint8 speed) 
{
    ...
    Out_Comp_CR = (Out_Comp_CR & (uint8)(~Out_Comp_PWR_MODE_MASK)) |
                           (speed & Out_Comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Out_Comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Out_Comp_TR = Out_Comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Out_Comp_TR0 = Out_Comp_HS_TRIM_TR0;
            Out_Comp_TR1 = Out_Comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Out_Comp_TR = Out_Comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Out_Comp_TR0 = Out_Comp_LS_TRIM_TR0;
            Out_Comp_TR1 = Out_Comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Out_Comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Out_Comp_WRK & Out_Comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Out_Comp_trimAdjust
********************************************************************************
*
* Summary:
...
static void Out_Comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Out_Comp_WRK & Out_Comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Out_Comp_TR |= Out_Comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Out_Comp_TR0 |= Out_Comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Out_Comp_TR |= Out_Comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Out_Comp_TR1 |= Out_Comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Out_Comp_WRK & Out_Comp_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Out_Comp_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Out_Comp_ZeroCal(void) 
{
    ...
    Out_Comp_SW0 = 0x00u;
    Out_Comp_SW2 = 0x00u;
    Out_Comp_SW3 = tmpSW3 & (uint8)(~Out_Comp_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Out_Comp_CR;
    Out_Comp_CR |= (Out_Comp_CAL_ON | Out_Comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Out_Comp_TR = Out_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Out_Comp_TR0 = Out_Comp_DEFAULT_CMP_TRIM;
        Out_Comp_TR1 = Out_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Out_Comp_CR & Out_Comp_PWR_MODE_MASK) == Out_Comp_PWR_MODE_FAST)
    {
        Out_Comp_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Out_Comp_trimAdjust(1u);
		Out_Comp_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Out_Comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Out_Comp_TR1 << 8u) | (Out_Comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Out_Comp_LoadTrim
********************************************************************************
*
* Summary:
...
void Out_Comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Out_Comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Out_Comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
