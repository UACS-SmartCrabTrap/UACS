...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_Start
********************************************************************************
*
* Summary:
...
void Bit_Timer_Clk_Start(void) 
{
    ...
    Bit_Timer_Clk_CLKEN |= Bit_Timer_Clk_CLKEN_MASK;
	Bit_Timer_Clk_CLKSTBY |= Bit_Timer_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Bit_Timer_Clk_Stop
********************************************************************************
*
* Summary:
...
void Bit_Timer_Clk_Stop(void) 
{
    ...
    Bit_Timer_Clk_CLKEN &= (uint8)(~Bit_Timer_Clk_CLKEN_MASK);
	Bit_Timer_Clk_CLKSTBY &= (uint8)(~Bit_Timer_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Bit_Timer_Clk_CLKEN & Bit_Timer_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Bit_Timer_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Bit_Timer_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Bit_Timer_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Bit_Timer_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Bit_Timer_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Bit_Timer_Clk_GetSourceRegister();
    uint16 oldDivider = Bit_Timer_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Bit_Timer_Clk_CLKEN & Bit_Timer_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Bit_Timer_Clk_DIV_PTR, clkDivider);
                Bit_Timer_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Bit_Timer_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Bit_Timer_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Bit_Timer_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Bit_Timer_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Bit_Timer_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Bit_Timer_Clk_MOD_SRC |= modeBitMask & (uint8)Bit_Timer_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Bit_Timer_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Bit_Timer_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Bit_Timer_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Bit_Timer_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Bit_Timer_Clk_MOD_SRC & (uint8)(Bit_Timer_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Bit_Timer_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Bit_Timer_Clk_GetDividerRegister();
    uint8 oldSrc = Bit_Timer_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Bit_Timer_Clk_MOD_SRC |= CYCLK_SSS;
        Bit_Timer_Clk_MOD_SRC =
            (Bit_Timer_Clk_MOD_SRC & (uint8)(~Bit_Timer_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Bit_Timer_Clk_MOD_SRC & Bit_Timer_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Bit_Timer_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Bit_Timer_Clk_PHASE = clkPhase & Bit_Timer_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Bit_Timer_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Bit_Timer_Clk_PHASE & Bit_Timer_Clk_PHASE_MASK;
}
