...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_Start
********************************************************************************
*
* Summary:
...
void LowF_ZeroCLK_Start(void) 
{
    ...
    LowF_ZeroCLK_CLKEN |= LowF_ZeroCLK_CLKEN_MASK;
	LowF_ZeroCLK_CLKSTBY |= LowF_ZeroCLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LowF_ZeroCLK_Stop
********************************************************************************
*
* Summary:
...
void LowF_ZeroCLK_Stop(void) 
{
    ...
    LowF_ZeroCLK_CLKEN &= (uint8)(~LowF_ZeroCLK_CLKEN_MASK);
	LowF_ZeroCLK_CLKSTBY &= (uint8)(~LowF_ZeroCLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LowF_ZeroCLK_CLKEN & LowF_ZeroCLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LowF_ZeroCLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LowF_ZeroCLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LowF_ZeroCLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LowF_ZeroCLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LowF_ZeroCLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LowF_ZeroCLK_GetSourceRegister();
    uint16 oldDivider = LowF_ZeroCLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LowF_ZeroCLK_CLKEN & LowF_ZeroCLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LowF_ZeroCLK_DIV_PTR, clkDivider);
                LowF_ZeroCLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LowF_ZeroCLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LowF_ZeroCLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LowF_ZeroCLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LowF_ZeroCLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: LowF_ZeroCLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LowF_ZeroCLK_MOD_SRC |= modeBitMask & (uint8)LowF_ZeroCLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: LowF_ZeroCLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LowF_ZeroCLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LowF_ZeroCLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: LowF_ZeroCLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LowF_ZeroCLK_MOD_SRC & (uint8)(LowF_ZeroCLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: LowF_ZeroCLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LowF_ZeroCLK_GetDividerRegister();
    uint8 oldSrc = LowF_ZeroCLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LowF_ZeroCLK_MOD_SRC |= CYCLK_SSS;
        LowF_ZeroCLK_MOD_SRC =
            (LowF_ZeroCLK_MOD_SRC & (uint8)(~LowF_ZeroCLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LowF_ZeroCLK_MOD_SRC & LowF_ZeroCLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LowF_ZeroCLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LowF_ZeroCLK_PHASE = clkPhase & LowF_ZeroCLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LowF_ZeroCLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LowF_ZeroCLK_PHASE & LowF_ZeroCLK_PHASE_MASK;
}
