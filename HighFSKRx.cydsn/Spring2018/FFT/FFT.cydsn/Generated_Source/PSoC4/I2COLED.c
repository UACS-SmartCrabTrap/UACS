...
uint8 I2COLED_initVar = 0u;
    ...
    * I2COLED_Stop() is called. Only these TX interrupt sources 
    * will be restored on a subsequent I2COLED_Enable() call.
    */
    uint16 I2COLED_IntrTxMask = 0u;
...
/** \} globals */
    ...
    void (*I2COLED_customIntrHandler)(void) = NULL;
...
static void I2COLED_ScbModePostEnable(void);
...
****************************************************************************//**
*
*  Initializes the I2COLED component to operate in one of the selected
*  configurations: I2C, SPI, UART or EZI2C.
*  When the configuration is set to "Unconfigured SCB", this function does
*  not do any initialization. Use mode-specific initialization APIs instead:
*  I2COLED_I2CInit, I2COLED_SpiInit, 
*  I2COLED_UartInit or I2COLED_EzI2CInit.
*
*******************************************************************************/
...
****************************************************************************//**
*
*  Enables I2COLED component operation: activates the hardware and 
*  internal interrupt. It also restores TX interrupt sources disabled after the 
*  I2COLED_Stop() function was called (note that level-triggered TX 
*  interrupt sources remain disabled to not cause code lock-up).
*  For I2C and EZI2C modes the interrupt is internal and mandatory for 
*  operation. For SPI and UART modes the interrupt can be configured as none, 
*  internal or external.
*  The I2COLED configuration should be not changed when the component
*  is enabled. Any configuration changes should be made after disabling the 
*  component.
*  When configuration is set to “Unconfigured I2COLED”, the component 
...
****************************************************************************//**
*
*  Invokes I2COLED_Init() and I2COLED_Enable().
...
****************************************************************************//**
*
*  Disables the I2COLED component: disable the hardware and internal 
*  interrupt. It also disables all TX interrupt sources so as not to cause an 
*  unexpected interrupt trigger because after the component is enabled, the 
*  TX FIFO is empty.
*  Refer to the function I2COLED_Enable() for the interrupt 
...
/*******************************************************************************
* Function Name: I2COLED_ScbModePostEnable
****************************************************************************//**
*
...
static void I2COLED_ScbModePostEnable(void)
{
    ...
    if (I2COLED_SCB_MODE_SPI_RUNTM_CFG)
    {
        I2COLED_SpiPostEnable();
    }
    else if (I2COLED_SCB_MODE_UART_RUNTM_CFG)
    {
        I2COLED_UartPostEnable();
    }
    else
    {
...
static void I2COLED_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void I2COLED_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            I2COLED_RX_SCL_MOSI_HSIOM_SEL_GPIO,
            I2COLED_TX_SDA_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (I2COLED_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_RX_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_PIN_DM_OD_LO;
        #elif (I2COLED_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX] = I2COLED_RX_WAKE_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX] = I2COLED_PIN_DM_OD_LO;
        #else
        #endif /* (I2COLED_RX_SCL_MOSI_PIN) */
        
        #if (I2COLED_TX_SDA_MISO_PIN)
            hsiomSel[I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_TX_SDA_MISO_HSIOM_SEL_I2C;
            pinsDm  [I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_PIN_DM_OD_LO;
        ...
        {
        #if (I2COLED_RX_SCL_MOSI_PIN)
            hsiomSel[I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_RX_SCL_MOSI_HSIOM_SEL_SPI;
        #elif (I2COLED_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX] = I2COLED_RX_WAKE_SCL_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (I2COLED_RX_SCL_MOSI_PIN) */
        
        #if (I2COLED_TX_SDA_MISO_PIN)
            hsiomSel[I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_TX_SDA_MISO_HSIOM_SEL_SPI;
        #endif /* (I2COLED_TX_SDA_MISO_PIN) */
        
        #if (I2COLED_SCLK_PIN)
            hsiomSel[I2COLED_SCLK_PIN_INDEX] = I2COLED_SCLK_HSIOM_SEL_SPI;
        #endif /* (I2COLED_SCLK_PIN) */

            if (I2COLED_SPI_SLAVE == subMode)
            {
                /* Slave */
                pinsDm[I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;
                pinsDm[I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_PIN_DM_STRONG;
                pinsDm[I2COLED_SCLK_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;

            #if (I2COLED_SS0_PIN)
                /* Only SS0 is valid choice for Slave */
                hsiomSel[I2COLED_SS0_PIN_INDEX] = I2COLED_SS0_HSIOM_SEL_SPI;
                pinsDm  [I2COLED_SS0_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;
            #endif /* (I2COLED_SS0_PIN) */

            #if (I2COLED_TX_SDA_MISO_PIN)
                /* Disable input buffer */
                 pinsInBuf |= I2COLED_TX_SDA_MISO_PIN_MASK;
            #endif /* (I2COLED_TX_SDA_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_PIN_DM_STRONG;
                pinsDm[I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;
            ...
            #if (I2COLED_RX_SCL_MOSI_PIN)
                pinsInBuf |= I2COLED_RX_SCL_MOSI_PIN_MASK;
            #elif (I2COLED_RX_WAKE_SCL_MOSI_PIN)
                pinsInBuf |= I2COLED_RX_WAKE_SCL_MOSI_PIN_MASK;
            #else
            #endif /* (I2COLED_RX_SCL_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (I2COLED_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (I2COLED_TX_SDA_MISO_PIN)
                hsiomSel[I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_TX_SDA_MISO_HSIOM_SEL_UART;
                pinsDm  [I2COLED_TX_SDA_MISO_PIN_INDEX] = I2COLED_PIN_DM_OD_LO;
            #endif /* (I2COLED_TX_SDA_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (I2COLED_RX_SCL_MOSI_PIN)
                    hsiomSel[I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_RX_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [I2COLED_RX_SCL_MOSI_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;
                #elif (I2COLED_RX_WAKE_SCL_MOSI_PIN)
                    hsiomSel[I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX] = I2COLED_RX_WAKE_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;
                #else
                #endif /* (I2COLED_RX_SCL_MOSI_PIN) */
                }
                ...
                if (I2COLED_UART_MODE_STD == subMode)
                {
                    ...
                    #if (I2COLED_SCLK_PIN)
                        hsiomSel[I2COLED_SCLK_PIN_INDEX] = I2COLED_SCLK_HSIOM_SEL_UART;
                        pinsDm  [I2COLED_SCLK_PIN_INDEX] = I2COLED_PIN_DM_DIG_HIZ;
                    #endif /* (I2COLED_SCLK_PIN) */
                    }
                    ...
                    #if (I2COLED_SS0_PIN)
                        hsiomSel[I2COLED_SS0_PIN_INDEX] = I2COLED_SS0_HSIOM_SEL_UART;
                        pinsDm  [I2COLED_SS0_PIN_INDEX] = I2COLED_PIN_DM_STRONG;
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        I2COLED_SET_HSIOM_SEL(I2COLED_RX_SCL_MOSI_HSIOM_REG,
                                       ...
                                       I2COLED_RX_SCL_MOSI_HSIOM_POS,
                                        hsiomSel[I2COLED_RX_SCL_MOSI_PIN_INDEX]);

        I2COLED_uart_rx_i2c_scl_spi_mosi_SetDriveMode((uint8) pinsDm[I2COLED_RX_SCL_MOSI_PIN_INDEX]);

        #if (!I2COLED_CY_SCBIP_V1)
            I2COLED_SET_INP_DIS(I2COLED_uart_rx_i2c_scl_spi_mosi_INP_DIS,
                                         I2COLED_uart_rx_i2c_scl_spi_mosi_MASK,
                                         (0u != (pinsInBuf & I2COLED_RX_SCL_MOSI_PIN_MASK)));
        #endif /* (!I2COLED_CY_SCBIP_V1) */
        ...
        I2COLED_SET_HSIOM_SEL(I2COLED_RX_WAKE_SCL_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX]);

        I2COLED_uart_rx_wake_i2c_scl_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[I2COLED_RX_WAKE_SCL_MOSI_PIN_INDEX]);

        I2COLED_SET_INP_DIS(I2COLED_uart_rx_wake_i2c_scl_spi_mosi_INP_DIS,
                                     I2COLED_uart_rx_wake_i2c_scl_spi_mosi_MASK,
                                     (0u != (pinsInBuf & I2COLED_RX_WAKE_SCL_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        I2COLED_SET_INCFG_TYPE(I2COLED_RX_WAKE_SCL_MOSI_INTCFG_REG,
                                        I2COLED_RX_WAKE_SCL_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (I2COLED_TX_SDA_MISO_PIN)
        I2COLED_SET_HSIOM_SEL(I2COLED_TX_SDA_MISO_HSIOM_REG,
                                       I2COLED_TX_SDA_MISO_HSIOM_MASK,
                                       I2COLED_TX_SDA_MISO_HSIOM_POS,
                                        hsiomSel[I2COLED_TX_SDA_MISO_PIN_INDEX]);

        I2COLED_uart_tx_i2c_sda_spi_miso_SetDriveMode((uint8) pinsDm[I2COLED_TX_SDA_MISO_PIN_INDEX]);

    #if (!I2COLED_CY_SCBIP_V1)
        I2COLED_SET_INP_DIS(I2COLED_uart_tx_i2c_sda_spi_miso_INP_DIS,
                                     I2COLED_uart_tx_i2c_sda_spi_miso_MASK,
                                    (0u != (pinsInBuf & I2COLED_TX_SDA_MISO_PIN_MASK)));
    #endif /* (!I2COLED_CY_SCBIP_V1) */
    #endif /* (I2COLED_RX_SCL_MOSI_PIN) */

    #if (I2COLED_SCLK_PIN)
        I2COLED_SET_HSIOM_SEL(I2COLED_SCLK_HSIOM_REG,
                                       I2COLED_SCLK_HSIOM_MASK,
                                       I2COLED_SCLK_HSIOM_POS,
                                       hsiomSel[I2COLED_SCLK_PIN_INDEX]);
    ...
    #if (I2COLED_SS0_PIN)
        I2COLED_SET_HSIOM_SEL(I2COLED_SS0_HSIOM_REG,
                                       I2COLED_SS0_HSIOM_MASK,
                                       I2COLED_SS0_HSIOM_POS,
                                       hsiomSel[I2COLED_SS0_PIN_INDEX]);
    ...
    #if (I2COLED_SS1_PIN)
        I2COLED_SET_HSIOM_SEL(I2COLED_SS1_HSIOM_REG,
                                       I2COLED_SS1_HSIOM_MASK,
                                       I2COLED_SS1_HSIOM_POS,
                                       hsiomSel[I2COLED_SS1_PIN_INDEX]);
    ...
    #if (I2COLED_SS2_PIN)
        I2COLED_SET_HSIOM_SEL(I2COLED_SS2_HSIOM_REG,
                                       I2COLED_SS2_HSIOM_MASK,
                                       I2COLED_SS2_HSIOM_POS,
                                       hsiomSel[I2COLED_SS2_PIN_INDEX]);
    ...
    #if (I2COLED_SS3_PIN)
        I2COLED_SET_HSIOM_SEL(I2COLED_SS3_HSIOM_REG,
                                       I2COLED_SS3_HSIOM_MASK,
                                       I2COLED_SS3_HSIOM_POS,
                                       hsiomSel[I2COLED_SS3_PIN_INDEX]);
