...
/*******************************************************************************
* Function Name: HighF_DelayClk_Start
********************************************************************************
*
* Summary:
...
void HighF_DelayClk_Start(void) 
{
    ...
    HighF_DelayClk_CLKEN |= HighF_DelayClk_CLKEN_MASK;
	HighF_DelayClk_CLKSTBY |= HighF_DelayClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HighF_DelayClk_Stop
********************************************************************************
*
* Summary:
...
void HighF_DelayClk_Stop(void) 
{
    ...
    HighF_DelayClk_CLKEN &= (uint8)(~HighF_DelayClk_CLKEN_MASK);
	HighF_DelayClk_CLKSTBY &= (uint8)(~HighF_DelayClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HighF_DelayClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HighF_DelayClk_CLKEN & HighF_DelayClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HighF_DelayClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HighF_DelayClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HighF_DelayClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HighF_DelayClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HighF_DelayClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HighF_DelayClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HighF_DelayClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HighF_DelayClk_GetSourceRegister();
    uint16 oldDivider = HighF_DelayClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HighF_DelayClk_CLKEN & HighF_DelayClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HighF_DelayClk_DIV_PTR, clkDivider);
                HighF_DelayClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HighF_DelayClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HighF_DelayClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HighF_DelayClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HighF_DelayClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HighF_DelayClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: HighF_DelayClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_DelayClk_MOD_SRC |= modeBitMask & (uint8)HighF_DelayClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: HighF_DelayClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_DelayClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HighF_DelayClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: HighF_DelayClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_DelayClk_MOD_SRC & (uint8)(HighF_DelayClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: HighF_DelayClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HighF_DelayClk_GetDividerRegister();
    uint8 oldSrc = HighF_DelayClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HighF_DelayClk_MOD_SRC |= CYCLK_SSS;
        HighF_DelayClk_MOD_SRC =
            (HighF_DelayClk_MOD_SRC & (uint8)(~HighF_DelayClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HighF_DelayClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_DelayClk_MOD_SRC & HighF_DelayClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HighF_DelayClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HighF_DelayClk_PHASE = clkPhase & HighF_DelayClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HighF_DelayClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HighF_DelayClk_PHASE & HighF_DelayClk_PHASE_MASK;
}
