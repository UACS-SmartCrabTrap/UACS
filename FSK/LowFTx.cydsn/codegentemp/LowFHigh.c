...
void LowFHigh_Start(void) 
{
    ...
    LowFHigh_CLKEN |= LowFHigh_CLKEN_MASK;
	LowFHigh_CLKSTBY |= LowFHigh_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LowFHigh_Stop
********************************************************************************
*
* Summary:
...
void LowFHigh_Stop(void) 
{
    ...
    LowFHigh_CLKEN &= (uint8)(~LowFHigh_CLKEN_MASK);
	LowFHigh_CLKSTBY &= (uint8)(~LowFHigh_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LowFHigh_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LowFHigh_CLKEN & LowFHigh_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LowFHigh_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LowFHigh_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LowFHigh__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LowFHigh_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LowFHigh_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LowFHigh_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LowFHigh_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LowFHigh_GetSourceRegister();
    uint16 oldDivider = LowFHigh_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LowFHigh_CLKEN & LowFHigh_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LowFHigh_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: LowFHigh_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LowFHigh_DIV_PTR);
}


/*******************************************************************************
* Function Name: LowFHigh_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LowFHigh_MOD_SRC |= modeBitMask & (uint8)LowFHigh_MODE_MASK;
}


/*******************************************************************************
* Function Name: LowFHigh_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LowFHigh_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LowFHigh_MODE_MASK));
}


/*******************************************************************************
* Function Name: LowFHigh_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LowFHigh_MOD_SRC & (uint8)(LowFHigh_MODE_MASK);
}


/*******************************************************************************
* Function Name: LowFHigh_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LowFHigh_GetDividerRegister();
    uint8 oldSrc = LowFHigh_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LowFHigh_MOD_SRC |= CYCLK_SSS;
        LowFHigh_MOD_SRC =
            (LowFHigh_MOD_SRC & (uint8)(~LowFHigh_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LowFHigh_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LowFHigh_MOD_SRC & LowFHigh_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LowFHigh_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LowFHigh_PHASE = clkPhase & LowFHigh_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LowFHigh_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LowFHigh_PHASE & LowFHigh_PHASE_MASK;
}
