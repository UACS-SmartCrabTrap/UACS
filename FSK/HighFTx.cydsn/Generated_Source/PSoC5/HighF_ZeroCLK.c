...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_Start
********************************************************************************
*
* Summary:
...
void HighF_ZeroCLK_Start(void) 
{
    ...
    HighF_ZeroCLK_CLKEN |= HighF_ZeroCLK_CLKEN_MASK;
	HighF_ZeroCLK_CLKSTBY |= HighF_ZeroCLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HighF_ZeroCLK_Stop
********************************************************************************
*
* Summary:
...
void HighF_ZeroCLK_Stop(void) 
{
    ...
    HighF_ZeroCLK_CLKEN &= (uint8)(~HighF_ZeroCLK_CLKEN_MASK);
	HighF_ZeroCLK_CLKSTBY &= (uint8)(~HighF_ZeroCLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HighF_ZeroCLK_CLKEN & HighF_ZeroCLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HighF_ZeroCLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HighF_ZeroCLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HighF_ZeroCLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HighF_ZeroCLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HighF_ZeroCLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HighF_ZeroCLK_GetSourceRegister();
    uint16 oldDivider = HighF_ZeroCLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HighF_ZeroCLK_CLKEN & HighF_ZeroCLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HighF_ZeroCLK_DIV_PTR, clkDivider);
                HighF_ZeroCLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HighF_ZeroCLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HighF_ZeroCLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HighF_ZeroCLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HighF_ZeroCLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HighF_ZeroCLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_ZeroCLK_MOD_SRC |= modeBitMask & (uint8)HighF_ZeroCLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HighF_ZeroCLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_ZeroCLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HighF_ZeroCLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HighF_ZeroCLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_ZeroCLK_MOD_SRC & (uint8)(HighF_ZeroCLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HighF_ZeroCLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HighF_ZeroCLK_GetDividerRegister();
    uint8 oldSrc = HighF_ZeroCLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HighF_ZeroCLK_MOD_SRC |= CYCLK_SSS;
        HighF_ZeroCLK_MOD_SRC =
            (HighF_ZeroCLK_MOD_SRC & (uint8)(~HighF_ZeroCLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_ZeroCLK_MOD_SRC & HighF_ZeroCLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HighF_ZeroCLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HighF_ZeroCLK_PHASE = clkPhase & HighF_ZeroCLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HighF_ZeroCLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HighF_ZeroCLK_PHASE & HighF_ZeroCLK_PHASE_MASK;
}
