...
void HighF_LevelCount_Init(void) 
{
        ...
        /* Check if One Shot mode is enabled i.e. RunMode !=0*/
            ...
            HighF_LevelCount_CONTROL |= 0x04u;
        #endif /* One Shot enabled only when RunModeUsed is not Continuous*/

        #if (HighF_LevelCount_RunModeUsed == 2)
            #if (CY_PSOC5A)
                /* Set last 2 bits of control2 register if one shot(halt on
                interrupt) is enabled*/
                HighF_LevelCount_CONTROL2 |= 0x03u;
            #endif /* Set One-Shot Halt on Interrupt bit in CONTROL2 for PSoC5A */

            #if (CY_PSOC3 || CY_PSOC5LP)
                /* Set last 2 bits of control3 register if one shot(halt on
                interrupt) is enabled*/
                HighF_LevelCount_CONTROL3 |= 0x03u;
            #endif /* Set One-Shot Halt on Interrupt bit in CONTROL3 for PSoC3 or PSoC5LP */

        #endif /* Remove section if One Shot Halt on Interrupt is not enabled */

        #if (HighF_LevelCount_UsingHWEnable != 0)
            #if (CY_PSOC5A)
                /* Set the default Run Mode of the Timer to Continuous */
                HighF_LevelCount_CONTROL2 |= HighF_LevelCount_CTRL_MODE_PULSEWIDTH;
            #endif /* Set Continuous Run Mode in CONTROL2 for PSoC5A */

            #if (CY_PSOC3 || CY_PSOC5LP)
                /* Clear and Set ROD and COD bits of CFG2 register */
                HighF_LevelCount_CONTROL3 &= ((uint8)(~HighF_LevelCount_CTRL_RCOD_MASK));
                HighF_LevelCount_CONTROL3 |= HighF_LevelCount_CTRL_RCOD;

                /* Clear and Enable the HW enable bit in CFG2 register */
                HighF_LevelCount_CONTROL3 &= ((uint8)(~HighF_LevelCount_CTRL_ENBL_MASK));
                HighF_LevelCount_CONTROL3 |= HighF_LevelCount_CTRL_ENBL;

                /* Set the default Run Mode of the Timer to Continuous */
                HighF_LevelCount_CONTROL3 |= HighF_LevelCount_CTRL_MODE_CONTINUOUS;
    ...
    HighF_LevelCount_WritePeriod(HighF_LevelCount_INIT_PERIOD);
    HighF_LevelCount_WriteCounter(HighF_LevelCount_INIT_PERIOD);

    #if (HighF_LevelCount_UsingHWCaptureCounter)/* Capture counter is enabled */
        HighF_LevelCount_CAPTURE_COUNT_CTRL |= HighF_LevelCount_CNTR_ENABLE;
        HighF_LevelCount_SetCaptureCount(HighF_LevelCount_INIT_CAPTURE_COUNT);
    #endif /* Configure capture counter value */

    #if (!HighF_LevelCount_UsingFixedFunction)
        #if (HighF_LevelCount_SoftwareCaptureMode)
            HighF_LevelCount_SetCaptureMode(HighF_LevelCount_INIT_CAPTURE_MODE);
        #endif /* Set Capture Mode for UDB implementation if capture mode is software controlled */

        #if (HighF_LevelCount_SoftwareTriggerMode)
            #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED)
                if (0u == (HighF_LevelCount_CONTROL & HighF_LevelCount__B_TIMER__TM_SOFTWARE))
                {
                    HighF_LevelCount_SetTriggerMode(HighF_LevelCount_INIT_TRIGGER_MODE);
                }
        ...
        CyExitCriticalSection(HighF_LevelCount_interruptState);
            ...
            HighF_LevelCount_EnableTrigger();
        #endif /* Set Trigger enable bit for UDB implementation in the control register*/
		
		
        #if (HighF_LevelCount_InterruptOnCaptureCount && !HighF_LevelCount_UDB_CONTROL_REG_REMOVED)
            HighF_LevelCount_SetInterruptCount(HighF_LevelCount_INIT_INT_CAPTURE_COUNT);
        #endif /* Set interrupt count in UDB implementation if interrupt count feature is checked.*/

        HighF_LevelCount_ClearFIFO();
    #endif /* Configure additional features of UDB implementation */

    HighF_LevelCount_SetInterruptMode(HighF_LevelCount_INIT_INTERRUPT_MODE);
}
...
void HighF_LevelCount_Enable(void) 
{
    ...
    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED || HighF_LevelCount_UsingFixedFunction)
        HighF_LevelCount_CONTROL |= HighF_LevelCount_CTRL_ENABLE;
...
{
    HighF_LevelCount_STATUS_MASK = interruptMode;
}


/*******************************************************************************
* Function Name: HighF_LevelCount_SoftwareCapture
********************************************************************************
*
* Summary:
...
#if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED) /* Remove API if control register is unused */
...
{
    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED) 
        return ((uint8)HighF_LevelCount_CONTROL);
    #else
        return (0);
    #endif /* (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED) */
}
...
void HighF_LevelCount_WritePeriod(uint16 period) 
{
        ...
        CY_SET_REG16(HighF_LevelCount_PERIOD_LSB_PTR, period_temp);
    #else
...
{
   #if(HighF_LevelCount_UsingFixedFunction)
       return ((uint16)CY_GET_REG16(HighF_LevelCount_CAPTURE_LSB_PTR));
   #else
       return (CY_GET_REG16(HighF_LevelCount_CAPTURE_LSB_PTR));
   #endif /* (HighF_LevelCount_UsingFixedFunction) */
}
...
#if(!HighF_LevelCount_UsingFixedFunction) /* UDB Specific Functions */
...
*  HighF_LevelCount__B_TIMER__CM_NONE        - Set Capture mode to None
*  HighF_LevelCount__B_TIMER__CM_RISINGEDGE  - Rising edge of Capture input
*  HighF_LevelCount__B_TIMER__CM_FALLINGEDGE - Falling edge of Capture input
*  HighF_LevelCount__B_TIMER__CM_EITHEREDGE  - Either edge of Capture input
*
* Return:
*  void
*
...
void HighF_LevelCount_SetCaptureMode(uint8 captureMode) 
{
    ...
    captureMode = ((uint8)((uint8)captureMode << HighF_LevelCount_CTRL_CAP_MODE_SHIFT));
    captureMode &= (HighF_LevelCount_CTRL_CAP_MODE_MASK);

    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        HighF_LevelCount_CONTROL &= ((uint8)(~HighF_LevelCount_CTRL_CAP_MODE_MASK));

        /* Write The New Setting */
        HighF_LevelCount_CONTROL |= captureMode;
    #endif /* (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED) */
}
...
/*******************************************************************************
* Function Name: HighF_LevelCount_SetTriggerMode
********************************************************************************
*
* Summary:
...
{
    /* This must only set to two bits of the control register associated */
    triggerMode &= HighF_LevelCount_CTRL_TRIG_MODE_MASK;

    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
    
        /* Clear the Current Setting */
        HighF_LevelCount_CONTROL &= ((uint8)(~HighF_LevelCount_CTRL_TRIG_MODE_MASK));

        /* Write The New Setting */
        HighF_LevelCount_CONTROL |= (triggerMode | HighF_LevelCount__B_TIMER__TM_SOFTWARE);
...
/*******************************************************************************
* Function Name: HighF_LevelCount_EnableTrigger
********************************************************************************
*
* Summary:
...
{
    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
        HighF_LevelCount_CONTROL |= HighF_LevelCount_CTRL_TRIG_EN;
    #endif /* Remove code section if control register is not used */
}


/*******************************************************************************
* Function Name: HighF_LevelCount_DisableTrigger
********************************************************************************
*
* Summary:
...
{
    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED )   /* Remove assignment if control register is removed */
        HighF_LevelCount_CONTROL &= ((uint8)(~HighF_LevelCount_CTRL_TRIG_EN));
    #endif /* Remove code section if control register is not used */
}
...
/*******************************************************************************
* Function Name: HighF_LevelCount_SetInterruptCount
********************************************************************************
*
* Summary:
...
{
    /* This must only set to two bits of the control register associated */
    interruptCount &= HighF_LevelCount_CTRL_INTCNT_MASK;

    #if (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        HighF_LevelCount_CONTROL &= ((uint8)(~HighF_LevelCount_CTRL_INTCNT_MASK));
        /* Write The New Setting */
        HighF_LevelCount_CONTROL |= interruptCount;
    #endif /* (!HighF_LevelCount_UDB_CONTROL_REG_REMOVED) */
}
...
/*******************************************************************************
* Function Name: HighF_LevelCount_SetCaptureCount
********************************************************************************
*
* Summary:
...
{
    HighF_LevelCount_CAP_COUNT = captureCount;
}


/*******************************************************************************
* Function Name: HighF_LevelCount_ReadCaptureCount
********************************************************************************
*
* Summary:
...
{
    return ((uint8)HighF_LevelCount_CAP_COUNT);
}
