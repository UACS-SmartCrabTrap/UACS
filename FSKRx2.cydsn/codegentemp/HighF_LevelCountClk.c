/*******************************************************************************
* File Name: HighF_LevelCountClk.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_Start
********************************************************************************
*
* Summary:
...
void HighF_LevelCountClk_Start(void) 
{
    ...
    HighF_LevelCountClk_CLKEN |= HighF_LevelCountClk_CLKEN_MASK;
	HighF_LevelCountClk_CLKSTBY |= HighF_LevelCountClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HighF_LevelCountClk_Stop
********************************************************************************
*
* Summary:
...
void HighF_LevelCountClk_Stop(void) 
{
    ...
    HighF_LevelCountClk_CLKEN &= (uint8)(~HighF_LevelCountClk_CLKEN_MASK);
	HighF_LevelCountClk_CLKSTBY &= (uint8)(~HighF_LevelCountClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HighF_LevelCountClk_CLKEN & HighF_LevelCountClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HighF_LevelCountClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HighF_LevelCountClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HighF_LevelCountClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HighF_LevelCountClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HighF_LevelCountClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HighF_LevelCountClk_GetSourceRegister();
    uint16 oldDivider = HighF_LevelCountClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HighF_LevelCountClk_CLKEN & HighF_LevelCountClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HighF_LevelCountClk_DIV_PTR, clkDivider);
                HighF_LevelCountClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HighF_LevelCountClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HighF_LevelCountClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HighF_LevelCountClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(HighF_LevelCountClk_DIV_PTR, clkDivider);
				HighF_LevelCountClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HighF_LevelCountClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: HighF_LevelCountClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_LevelCountClk_MOD_SRC |= modeBitMask & (uint8)HighF_LevelCountClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: HighF_LevelCountClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HighF_LevelCountClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HighF_LevelCountClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: HighF_LevelCountClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_LevelCountClk_MOD_SRC & (uint8)(HighF_LevelCountClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: HighF_LevelCountClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HighF_LevelCountClk_GetDividerRegister();
    uint8 oldSrc = HighF_LevelCountClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HighF_LevelCountClk_MOD_SRC |= CYCLK_SSS;
        HighF_LevelCountClk_MOD_SRC =
            (HighF_LevelCountClk_MOD_SRC & (uint8)(~HighF_LevelCountClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HighF_LevelCountClk_MOD_SRC & HighF_LevelCountClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HighF_LevelCountClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HighF_LevelCountClk_PHASE = clkPhase & HighF_LevelCountClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HighF_LevelCountClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HighF_LevelCountClk_PHASE & HighF_LevelCountClk_PHASE_MASK;
}
